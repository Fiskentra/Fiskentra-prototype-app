...
public final class DesignScreens {
    ...
    public static final class State {
        ...
        public int pending, setupStep;
        public long tripStarted, tripStopped;
    ...
    public View navigation(String screen) {
        ...
        int selected = screen.equals("saved") || screen.equals("catchEdit") ? 2
    ...
    private View buildMapScreen(boolean tools, boolean active) {
        ...
        if(tools){LinearLayout sheet=panel(10);section(sheet,"Map & layers","");
            sheet.addView(toggle(R.drawable.ic_map,"Offline map",false,null));
            ...
            LinearLayout tools2=row();tools2.addView(tool(R.drawable.ic_download,"Export GPX",()->soon("GPX export")),weight());tools2.addView(tool(R.drawable.ic_map,"Download area",()->soon("Offline map download")),weight());tools2.addView(tool(R.drawable.ic_microphone,"Voice",()->soon("Voice navigation")),weight());sheet.addView(tools2,gap(8));
        ...
        }else if(active){LinearLayout actions=col();actions.setPadding(dp(8),dp(6),dp(8),dp(6));actions.addView(fieldButton("CATCH","1 PRESS",R.drawable.ic_fish,LIME,()->host.command("save:Catch")),gap(5));actions.addView(fieldButton("WAYPOINT","2 PRESSES",R.drawable.ic_map_pin,AMBER,()->host.command("save:Waypoint")),gap(5));actions.addView(fieldButton("TACKLE CHANGE","HOLD",R.drawable.ic_fish_hook,PURPLE,()->host.command("save:Tackle change")),gap(5));LinearLayout bottom=row();bottom.addView(tool(R.drawable.ic_player_pause,"Pause",()->soon("Pause track")),weight());bottom.addView(tool(R.drawable.ic_alert_triangle,"SOS / Share",()->soon("SOS sharing")),weight());bottom.addView(tool(R.drawable.ic_player_stop,"Finish",()->host.command("finishTrip")),weight());actions.addView(bottom);root.addView(actions);
    ...
    private String pointName(SavedPoint p){return p.catchDetails!=null&&!p.catchDetails.species.isEmpty()?p.catchDetails.species:p.type;}
...
private View buildDevices(){LinearLayout root=base(false,false),body=scrollBody(root,10);LinearLayout device=panel(8);device.addView(infoRow(R.drawable.ic_bluetooth,s.device.isEmpty()?"Flic 2":s.device,s.connected?"Connected":"Not connected",null));LinearLayout buttons=row();buttons.addView(button("CONFIGURE",0,false,()->host.go("flicSetup")),weight());buttons.addView(button("ADD DEVICE",0,false,()->host.command("pair")),weight());device.addView(buttons);body.addView(device,gap(8));section(body,"Flic 2 button mapping","");LinearLayout mapping=panel(4);mapping.addView(infoRow(R.drawable.ic_fish,"1 press","Catch",()->soon("Button remapping")));mapping.addView(divider());mapping.addView(infoRow(R.drawable.ic_map_pin,"2 presses","Waypoint",()->soon("Button remapping")));mapping.addView(divider());mapping.addView(infoRow(R.drawable.ic_fish_hook,"Hold","Tackle change",()->soon("Button remapping")));mapping.addView(button("TEST PRESSES",0,false,()->host.command("testFlic")));body.addView(mapping,gap(7));section(body,"App settings","");LinearLayout settings=panel(3);settings.addView(infoRow(R.drawable.ic_map,"Offline maps","Coming soon",()->host.go("mapTools")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_cloud,"Cloud backup",s.pending==0?"Synced":s.pending+" queued",()->host.command("sync")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_user,"Profile & sign-in",s.signedIn?s.name:"Local mode",()->host.go("profile")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_world,"Units & language",s.imperial?"Imperial · English":"Metric · English",()->unitDialog()));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_map_pin,"GPS / Bluetooth permissions",s.gps&&s.bluetooth?"Allowed":"Review",()->host.command("permissions")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_sun,"Forecast providers","Open-Meteo",()->host.go("forecastDetail")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_gauge,"Barometer offset","Coming soon",()->soon("Barometer calibration")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_alert_triangle,"Safety & SOS","Configure",()->soon("Safety & SOS")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_notebook,"News & offline library","Coming soon",()->soon("Offline library")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_help,"Help & tutorials","",()->host.go("flicSetup")));body.addView(settings,gap(8));body.addView(infoCard(R.drawable.ic_shield_check,"Field readiness",s.gps&&s.bluetooth?"Permissions ready":"Review permissions",()->host.go("beta")),gap(5));body.addView(text(s.status,12,MUTED,false));return root;}
    ...
    private View buildSummary(){
        ...
        if(catches.isEmpty())body.addView(text("No catches recorded for this trip.",14,MUTED,false),gap(14));
            ...
            for(SavedPoint point:catches){ImageView photo=pointPhoto(point,82);photo.setOnClickListener(v->host.point(point,"edit"));LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(dp(94),dp(82));lp.rightMargin=dp(4);photos.addView(photo,lp);}
            ...
            card.setOnClickListener(v->host.point(selected,"edit"));body.addView(card,gap(10));
        }
    ...
    private View buildFlicSetup(){LinearLayout root=authBase(),body=scrollBody(root,18);section(body,"Set up your Flic 2","Step "+(s.setupStep+1)+" of 4");LinearLayout progress=row();for(int i=0;i<4;i++){View segment=new View(a);segment.setBackgroundColor(i<=s.setupStep?BLUE:LINE);LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(3),1);p.setMargins(dp(2),0,dp(2),dp(18));progress.addView(segment,p);}body.addView(progress);String[] titles={"Ready for the water","Connect your button","Test your Flic 2","You're ready to fish"};body.addView(text(titles[Math.max(0,Math.min(3,s.setupStep))],28,INK,true),gap(14));
        ...
        else if(s.setupStep==2){body.addView(text("Press the physical button. These test presses will not create journal entries.",16,MUTED,false),gap(20));testRow(body,R.drawable.ic_fish,"Single press","Catch",s.singleTest);testRow(body,R.drawable.ic_map_pin,"Double press","Waypoint",s.doubleTest);testRow(body,R.drawable.ic_fish_hook,"Press and hold","Tackle change",s.holdTest);body.addView(text(s.connected?"Listening for your Flic 2…":"Reconnect Flic 2 to continue.",14,s.connected?BLUE:AMBER,false),gap(18));ready=s.singleTest&&s.doubleTest&&s.holdTest;}
        else{ImageView readyImage=new ImageView(a);readyImage.setImageResource(R.drawable.flic2_setup_hero);readyImage.setScaleType(ImageView.ScaleType.FIT_CENTER);body.addView(readyImage,new LinearLayout.LayoutParams(-1,dp(170)));body.addView(icon(R.drawable.ic_circle_check,LIME,42),gap(12));body.addView(text(s.connected&&s.singleTest&&s.doubleTest&&s.holdTest?"Your button is connected and tested. Use one press for a catch, two for a waypoint, and hold for a tackle change.":"Review the connection and test results below before heading out.",17,MUTED,false),gap(20));body.addView(infoCard(R.drawable.ic_bluetooth,"Flic 2",s.connected?"Connected":"Disconnected",null),gap(8));body.addView(infoCard(R.drawable.ic_fish,"Single press",s.singleTest?"Passed":"Not tested",null),gap(8));body.addView(infoCard(R.drawable.ic_map_pin,"Double press",s.doubleTest?"Passed":"Not tested",null),gap(8));body.addView(infoCard(R.drawable.ic_fish_hook,"Hold",s.holdTest?"Passed":"Not tested",null),gap(8));body.addView(infoCard(R.drawable.ic_current_location,"GPS",gps(),null),gap(8));ready=true;}
