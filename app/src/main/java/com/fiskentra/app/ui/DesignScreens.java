...
public final class DesignScreens {
    ...
    public static final class State {
        ...
        public List<double[]> tripTrack = Collections.emptyList();
        ...
        public boolean connected, bluetooth, gps, gpsEnabled, notifications, active, paused, signedIn, busy, authError, imperial;
    ...
    public View navigation(String screen) {
        LinearLayout bar = row(); bar.setBackgroundColor(0xff001e2e);
        int[] icons = {R.drawable.ic_map, R.drawable.ic_notebook, R.drawable.ic_bookmark, R.drawable.ic_bluetooth};
        String[] labels = {"Map", "Journal", "Saved", "Devices"};
        String[] routes = {"map", "log", "saved", "device"};
        ...
        for (int n = 0; n < 4; n++) {
            ...
            tab.addView(icon(icons[n], n == selected ? BLUE : 0xffb7c9e6, 24));
            TextView label = text(labels[n], 11, n == selected ? BLUE : 0xffb7c9e6, false); label.setTypeface(Typeface.DEFAULT);
            label.setGravity(Gravity.CENTER);label.setSingleLine(true);label.setHorizontallyScrolling(false);label.setPadding(dp(2),0,dp(2),0);
            label.setAutoSizeTextTypeUniformWithConfiguration(10,14,1,android.util.TypedValue.COMPLEX_UNIT_SP);
            tab.addView(label,new LinearLayout.LayoutParams(-1,dp(24)));
            View underline = new View(a); underline.setBackgroundColor(n == selected ? BLUE : Color.TRANSPARENT);
            LinearLayout.LayoutParams underlineLp = new LinearLayout.LayoutParams(dp(28), dp(2)); underlineLp.topMargin = dp(3); tab.addView(underline, underlineLp);
    ...
    private View tripSetup() {
        ...
        status.addView(infoRow(R.drawable.ic_cloud, "Cloud sync", s.pending == 0 ? a.getString(R.string.data_no_cloud_operations) : s.pending + " queued", () -> host.command("sync")));
    ...
    private View buildMapScreen(boolean tools, boolean active) {
        ...
        if(tools){LinearLayout sheet=panel(10);section(sheet,"Map & layers","");
            ...
            LinearLayout tools2=row();tools2.addView(tool(R.drawable.ic_download,"Export GPX",()->host.command("exportGpx")),weight());tools2.addView(tool(R.drawable.ic_map,"Download area",()->host.go("offlineMaps")),weight());tools2.addView(tool(R.drawable.ic_microphone,"Voice",()->soon("Voice navigation")),weight());sheet.addView(tools2,gap(8));
        ...
        }else if(active){LinearLayout actions=col();actions.setPadding(dp(8),dp(6),dp(8),dp(6));actions.addView(fieldButton("WAYPOINT","1 PRESS",R.drawable.ic_map_pin,AMBER,()->host.command("save:Waypoint")),gap(5));actions.addView(fieldButton("CATCH","2 PRESSES",R.drawable.ic_fish,LIME,()->host.command("save:Catch")),gap(5));actions.addView(fieldButton(s.paused?"RESUME TRACK":"PAUSE TRACK","HOLD",R.drawable.ic_route,PURPLE,()->host.command("toggleTrack")),gap(5));TextView background=text(s.paused?"● Track recording paused":"● Route recording continues with the screen locked",11,s.paused?AMBER:LIME,false);background.setGravity(Gravity.CENTER);actions.addView(background,gap(5));LinearLayout bottom=row();bottom.addView(tool(R.drawable.ic_player_pause,s.paused?"Resume":"Pause",()->host.command("toggleTrack")),weight());bottom.addView(tool(R.drawable.ic_alert_triangle,"SOS / Share",()->soon("SOS sharing")),weight());bottom.addView(tool(R.drawable.ic_player_stop,"Finish",()->host.command("finishTrip")),weight());actions.addView(bottom);root.addView(actions);
    ...
    private View journalItem(SavedPoint p){LinearLayout card=panel(6),line=row();LinearLayout day=col();day.setGravity(Gravity.CENTER);day.addView(text(date(p.timestamp,"d"),22,INK,false));day.addView(text(date(p.timestamp,"EEE"),11,MUTED,false));line.addView(day,new LinearLayout.LayoutParams(dp(32),-1));ImageView photo=pointPhoto(p,68);line.addView(photo,new LinearLayout.LayoutParams(dp(68),dp(68)));LinearLayout copy=col();copy.setPadding(dp(8),0,0,0);copy.addView(text(pointName(p),16,INK,true));copy.addView(text(date(p.timestamp,"HH:mm")+" · "+p.type,12,MUTED,false));if(!p.hasLocation())copy.addView(text(a.getString(R.string.data_no_location),12,AMBER,false));copy.addView(text(p.weather==null?"Weather not captured":temperature(p.weather.temperatureC)+", "+p.weather.condition(),12,MUTED,false));copy.addView(text(host.syncLabel(p),11,host.syncColor(p),false));line.addView(copy,new LinearLayout.LayoutParams(0,-2,1));line.addView(icon(pointIcon(p.type),pointColor(p.type),20));card.addView(line);card.setOnClickListener(v->host.point(p,"open"));return card;}
    ...
    private ImageView pointPhoto(SavedPoint p,int size) {
        ImageView photo=new ImageView(a);
        photo.setLayoutParams(new LinearLayout.LayoutParams(-1,dp(size)));
        photo.setImageResource(pointIcon(p.type));photo.setColorFilter(pointColor(p.type));
        photo.setScaleType(ImageView.ScaleType.CENTER_INSIDE);photo.setPadding(dp(15),dp(15),dp(15),dp(15));
        photo.setBackgroundColor(PANEL);photo.setContentDescription(p.type);
        String path=p.catchDetails==null?"":p.catchDetails.localPhotoPath;
        if(!path.isEmpty())LocalCatchPhoto.bind(photo,path,Math.max(dp(size),dp(120)));
        return photo;
    }
    ...
    private View buildOfflineMaps(){
        ...
        if(s.offlineAvailable){
            ...
            if(s.offlineComplete)body.addView(button("OPEN DOWNLOADED AREA",R.drawable.ic_map,true,()->host.command("offlineOpen")),gap(7));
            ...
            section(body,"Download around selected center","Choose radius");LinearLayout choices=row();for(int radius:new int[]{2,5,10}){final int value=radius;choices.addView(chip(radius+" km",offlineRadiusKm==radius,()->{offlineRadiusKm=value;host.go("offlineMaps");}),weight());}body.addView(choices,gap(9));
            LinearLayout details=panel(4);details.addView(infoRow(R.drawable.ic_map_pin,"Download center",s.location==null?"Choose an area on Map":String.format(Locale.US,"%.4f, %.4f",s.location.getLatitude(),s.location.getLongitude()),null));details.addView(divider());details.addView(infoRow(R.drawable.ic_layers,"Map style",MapTilerMapView.styleName(s.mapStyle),null));details.addView(divider());details.addView(infoRow(R.drawable.ic_download,"Zoom range","8–16",null));body.addView(details,gap(8));
...
private View buildDevices(){LinearLayout root=base(false,false),body=scrollBody(root,10);LinearLayout device=panel(8);device.addView(infoRow(R.drawable.ic_bluetooth,s.device.isEmpty()?"Flic 2":s.device,s.connected?"Connected":"Not connected",null));LinearLayout buttons=row();buttons.addView(button("CONFIGURE",0,false,()->host.go("flicSetup")),weight());buttons.addView(button("ADD DEVICE",0,false,()->host.command("pair")),weight());device.addView(buttons);body.addView(device,gap(8));section(body,"Flic 2 button mapping","");LinearLayout mapping=panel(4);mapping.addView(infoRow(R.drawable.ic_map_pin,"1 press","Waypoint",()->soon("Button remapping")));mapping.addView(divider());mapping.addView(infoRow(R.drawable.ic_fish,"2 presses","Catch",()->soon("Button remapping")));mapping.addView(divider());mapping.addView(infoRow(R.drawable.ic_route,"Hold","Start / pause / resume track",null));mapping.addView(button("TEST PRESSES",0,false,()->host.command("testFlic")));body.addView(mapping,gap(7));section(body,"App settings","");LinearLayout settings=panel(3);settings.addView(infoRow(R.drawable.ic_map,"Offline maps",s.offlineComplete?"Ready":s.offlineDownloading?s.offlineProgress+"%":s.offlineAvailable?"Paused":"Set up",()->host.go("offlineMaps")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_cloud,"Cloud backup",s.pending==0?"Synced":s.pending+" queued",()->host.command("sync")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_user,"Profile & sign-in",s.signedIn?s.name:"Local mode",()->host.go("profile")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_world,"Units & language",s.imperial?"Imperial · English":"Metric · English",()->unitDialog()));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_map_pin,"GPS / Bluetooth permissions",s.gps&&s.bluetooth?"Allowed":"Review",()->host.command("permissions")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_sun,"Forecast providers","Open-Meteo",()->host.go("forecastDetail")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_gauge,"Barometer offset","Coming soon",()->soon("Barometer calibration")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_alert_triangle,"Safety & SOS","Configure",()->soon("Safety & SOS")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_notebook,"News & offline library","Coming soon",()->soon("Offline library")));settings.addView(divider());settings.addView(infoRow(R.drawable.ic_help,"Help & tutorials","",()->host.go("flicSetup")));body.addView(settings,gap(8));body.addView(infoCard(R.drawable.ic_shield_check,"Field readiness",s.gps&&s.bluetooth?"Permissions ready":"Review permissions",()->host.go("beta")),gap(5));body.addView(text(s.status,12,MUTED,false));return root;}
    ...
    private View buildSummary(){
        ...
        if(catches.isEmpty())body.addView(text("No catches recorded for this trip.",14,MUTED,false),gap(14));
            ...
            LinearLayout position=row();position.addView(summaryValue("Location",pointCoordinates(best)),new LinearLayout.LayoutParams(0,-2,2));position.addView(summaryValue("Time",date(best.timestamp,"HH:mm")),weight());card.addView(position);
        ...
        body.addView(button("SHARE REPORT",R.drawable.ic_share,false,()->soon("Trip report sharing")),gap(8));body.addView(button("EXPORT GPX",R.drawable.ic_download,false,()->host.command("exportGpx")),gap(0));return root;
    }
...
private void testRow(LinearLayout body,int id,String title,String value,boolean passed){int color=id==R.drawable.ic_fish?LIME:id==R.drawable.ic_map_pin?AMBER:PURPLE;LinearLayout card=row();card.setGravity(Gravity.CENTER_VERTICAL);card.setPadding(dp(10),dp(14),dp(10),dp(14));card.setBackground(shape(PANEL,color,4));ImageView actionIcon=icon(id,color,42);actionIcon.setScaleType(ImageView.ScaleType.FIT_CENTER);card.addView(actionIcon);LinearLayout copy=col();copy.setPadding(dp(10),0,0,0);copy.addView(text(title.toUpperCase(Locale.US),18,color,false));copy.addView(text(value,14,INK,false));card.addView(copy,new LinearLayout.LayoutParams(0,-2,1));LinearLayout result=col();result.setGravity(Gravity.CENTER);result.addView(icon(passed?R.drawable.ic_circle_check:R.drawable.ic_clock,passed?LIME:MUTED,28));TextView status=text(passed?"Passed":"Waiting",12,passed?LIME:MUTED,false);status.setGravity(Gravity.CENTER);result.addView(status);card.addView(result,new LinearLayout.LayoutParams(dp(64),-2));body.addView(card,gap(12));}
    ...
    private String coords(){return s.selected==null&&s.location==null&&!s.points.isEmpty()?pointCoordinates(s.points.get(0)):s.selected!=null?pointCoordinates(s.selected):s.location==null?"Waiting for location":String.format(Locale.US,"%.5f, %.5f",s.location.getLatitude(),s.location.getLongitude());}
    ...
    private String pointCoordinates(SavedPoint point) {
        return point != null && point.hasLocation() ? String.format(Locale.US,"%.5f, %.5f",point.latitude,point.longitude) : a.getString(R.string.data_no_location);
    }
